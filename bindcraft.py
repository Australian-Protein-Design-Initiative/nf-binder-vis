...
def find_bindcraft_results(
    root_path: Path, exclude_folders: List[str] = None
) -> List[Path]:
    ...
    for dirpath, dirnames, filenames in os.walk(root_path):
        ...
        if current_dir.name in exclude_folders:
...
def load_bindcraft_data(
    root_search_path: Path, exclude_folders: List[str] = None
    ...
    if not root_search_path.exists() or not root_search_path.is_dir():
        st.error(
            f"Root search path {root_search_path} does not exist or is not a directory."
        )
        return None

    result_dirs = find_bindcraft_results(root_search_path, exclude_folders)
    ...
    st.info(
        f"Found {len(result_dirs)} BindCraft result director(y/ies). Aggregating data..."
    )
    ...
    for res_dir in result_dirs:
        ...
        accepted_dir = (
            res_dir / "Accepted"
        )  # Define accepted_dir for this result directory
        try:
            df_single = pd.read_csv(stats_file)
            # Add the relative path of the result directory
            df_single["results_dir_path"] = str(
                res_dir.relative_to(root_search_path)
                if res_dir.is_relative_to(root_search_path)
                else res_dir
            )
            ...
            if "Design" not in df_single.columns:
                st.error(
                    f"'Design' column not found in {stats_file}. Cannot locate PDB files."
                )
                ...
                for design_name in df_single["Design"]:
                    if pd.isna(
                        design_name
                    ):  # Handle cases where design_name might be NaN
                        pdb_file_paths.append(None)
                        st.warning(
                            f"Encountered a missing Design name in {stats_file}, cannot search for PDB."
                        )
                        continue
                    ...
                    if not accepted_dir.is_dir():
                        st.warning(
                            f"Accepted directory not found: {accepted_dir}. Cannot search PDB for {design_name}."
                        )
                        pdb_file_paths.append(None)
                        continue
                    ...
                    if exact_match_path.exists():
                        ...
                        potential_pdbs = sorted(
                            list(accepted_dir.glob(f"{design_name}_*.pdb"))
                        )

                        if not potential_pdbs:
                            # As a last resort, try the original glob pattern
                            potential_pdbs = sorted(
                                list(accepted_dir.glob(f"{design_name}*.pdb"))
                            )

                        if not potential_pdbs:
                            st.warning(
                                f"No PDB file found for Design '{design_name}' in {accepted_dir}"
                            )
                            ...
                            st.warning(
                                f"Multiple PDBs found for Design '{design_name}' in {accepted_dir}: {[p.name for p in potential_pdbs]}. Using the first one: {potential_pdbs[0].name}"
                            )
                            chosen_pdb_path = str(potential_pdbs[0].resolve())
            ...
            st.error(
                f"Error processing data from result directory {res_dir} (stats file: {stats_file}): {str(e)}"
            )
            # Optionally, you might want to skip this df_single or handle more gracefully

    if not all_dfs:
        st.error(
            "Failed to load or process data from any of the found BindCraft result directories."
        )
        return None
    ...
    if "pdb_file" not in combined_df.columns:
        combined_df["pdb_file"] = pd.Series(
            [None] * len(combined_df), dtype=pd.StringDtype()
        )
    else:
    ...
    )  # Avoid converting our StringDtype pdb_file back
    ...
    if "good" not in combined_df.columns:
        ...
        combined_df.insert(
            insert_pos, "good", False
        )  # Pandas will broadcast False to all rows
    ...
    if DEFAULT_SORT_COLUMN in combined_df.columns:
        combined_df = combined_df.sort_values(
            DEFAULT_SORT_COLUMN, ascending=DEFAULT_SORT_ASCENDING
        ).reset_index(drop=True)
    else:
        st.warning(
            f"'{DEFAULT_SORT_COLUMN}' column not found in the aggregated data. Cannot sort by it."
        )
...
def plot_scatter(df: pd.DataFrame, x_col: str, y_col: str) -> Optional[Any]:
    ...
    selection = alt.selection_point(
        name="selected_point", fields=["Design", x_col, y_col]
    )
...
def main():
            ...
            with st.spinner("Regenerating summary file..."):
                ...
                if fresh_df is not None:
                    ...
                    if (
                        ...
                        and "Design" in current_summary_df.columns
                        and "results_dir_path" in current_summary_df.columns
                        ...
                        good_stamps_to_merge = current_summary_df[
                            ...
                            subset=["Design", "results_dir_path"], keep="first"
                        )

                        if (
                            "good" in fresh_df.columns
                        ):  # Should exist due to above ensure block
    ...
    ):  # Only proceed if df is not already set (e.g., by regeneration button)
        if summary_file_path.exists():
            try:
                ...
                if "good" in st.session_state.df.columns:
                    ...
                    st.session_state.df["good"] = (
                        False  # Add good column if missing, pandas will broadcast
                    )
                ...
                if "results_dir_path" in st.session_state.df.columns:
                    st.session_state.df["results_dir_path"] = st.session_state.df[
                        "results_dir_path"
                    ].astype(pd.StringDtype())
    ...
    if df is None:
        st.warning(
            "No BindCraft data loaded. Please ensure the specified path contains valid BindCraft results or a summary file."
        )
        return
    ...
    if "good" in df.columns:
        ...
        df["good"] = (
            st.session_state.good_values
        )  # Add good column to df if it's missing
    ...
    with table_tab:
        if st.session_state.show_good_only:
            st.info(
                f"Displaying {len(df_display)} of {len(df)} total rows (good only)."
            )
        else:
    ...
    if edited_df_from_editor is not None:
        ...
        if "good" in edited_df_from_editor.columns and "good" in df.columns:
            ...
            for (
                ...
                changed_good_series.index
            ):  # original_df_idx here is the index from the DataFrame
                if original_df_idx in df.index:  # Ensure index is valid for original df
                    ...
                    if current_good_value_in_state != new_good_value_bool:
                        ...
                        new_selection_made_in_this_run = (
                            True  # Mark that a change was made
                        )
    ...
    if scatter_event_result:
        ...
        if selection_data and "selected_point" in selection_data:
            ...
            if selected_points and len(selected_points) > 0:
                ...
                if selected_design:
                    matching_rows = df[
                        df["Design"] == selected_design
                    ]  # Changed "description" to "Design"
    ...
    if not df.empty:
        ...
        with col_info_display:
            if (
                ...
                metric_value_for_display = selected_row_for_info.get(
                    DEFAULT_SORT_COLUMN, "N/A"
                )
                metric_text = "N/A"
                try:
                    ...
                    metric_text = (
                        str(metric_value_for_display)
                        if pd.notna(metric_value_for_display)
                        else "N/A"
                    )
                results_dir_path_text = selected_row_for_info.get(
                    "results_dir_path", "N/A"
                )
                ...
                table_data = {
                    "Attribute": [
                        ...
                        "Model:",
                        "Run:",
                        f"{DEFAULT_SORT_COLUMN}",
                        f"Good ({len(df[df['good'] == True])}/{len(df)}):",
                    ],
                    "Value": [
                        f"{rank_value}/{len(df)}",
                        design_name,
                        results_dir_path_text,
                        metric_text,
                        good_status_emoji,
                    ],
                }
            ...
            if isinstance(idx, int) and 0 <= idx < len(df):
                ...
                if pdb_file_path_str:
                    ...
                    if pdb_path.exists():
                        ...
                        st.warning(
                            f"PDB file not found for {design_name_for_warning}: {pdb_path}"
                        )
                else:
                    st.warning(
                        f"Row for {design_name_for_warning} (index {idx}) is missing 'pdb_file' information."
                    )
            else:
